...
public:
    ...
    Q_INVOKABLE bool updateGroup(const QString& id, const QString& name, const QString& colorHex = QStringLiteral("#337fe6"));
    Q_INVOKABLE bool deleteGroup(const QString& id);
    Q_INVOKABLE QVariantList loadWords(const QString& sortMode = QStringLiteral("newest"), const QString& groupId = QString());
    Q_INVOKABLE QVariantList searchWords(const QString& query, const QString& sortMode = QStringLiteral("newest"), const QString& groupId = QString());
    Q_INVOKABLE int wordCount(const QString& groupId = QString());
    Q_INVOKABLE int groupCount();
    Q_INVOKABLE qint64 databaseSize();
    Q_INVOKABLE QVariantMap databaseStats();
    Q_INVOKABLE QVariantMap wordById(const QString& id);
    Q_INVOKABLE QString createWord(const QVariantMap& wordData);
    Q_INVOKABLE bool updateWord(const QString& id, const QVariantMap& wordData);
    Q_INVOKABLE bool deleteWord(const QString& id);
    ...
    Q_INVOKABLE void openWordDetails(const QString& id);
    Q_INVOKABLE void openEditWord(const QString& id);
...
private:
    ...
    void setSelectedWordId(const QString& id);
